...
class JointControllerNode(Node):
    ...
    def __init__(self, node_name: str, sub_topic: str, pub_topic: str, area: JointArea, dofs: int):
        ...
        self.dofs = dofs
        self.joint_name_to_index = {joint.name: i for i, joint in enumerate(self.joint_info)}
        self.received_joint_state = [False] * dofs
    ...
    def joint_state_callback(self, msg: JointStateArray):
        ...
        with self.lock:
            for state in msg.joints:
                idx = self.joint_name_to_index.get(state.name)
                if idx is None:
                    continue
                self.input.current_position[idx] = state.position
                self.input.current_velocity[idx] = state.velocity
                self.input.current_acceleration[idx] = 0.0
                self.received_joint_state[idx] = True

            if all(self.received_joint_state):
    ...
    def set_target_position(self, joint_name, position):
        ...
        if not self.ruckig_initialized:
            self.get_logger().warn("Joint state not ready yet; skip command until first full state is received.")
            return

        joint_idx = self.joint_name_to_index.get(joint_name)
        if joint_idx is None:
            self.get_logger().error(f"Unknown joint name: {joint_name}")
            return

        with self.lock:
            # Keep non-target joints at their current positions to avoid unintended movement.
            p_s = list(self.input.current_position)
            p_s[joint_idx] = position
